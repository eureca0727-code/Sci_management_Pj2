...
- group_id는 절대 포함하지 말 것 (그룹 배정은 별도 단계에서 처리됨)
""".strip()
...
합의 기준 (우선순위 순):
1. 강의 범위 이탈 여부 (범위 이탈 시만 탈락)
2. 난이도 적절성 (블루프린트 기준)
3. 단원별 short_answer/essay/application 문제 수가 블루프린트와 일치
4. 중복 개념 제거 (같은 세션에 제출된 문제끼리만 — 완전히 동일한 개념일 때만 탈락)

[실패 패턴 처리 원칙 — 반드시 준수]
- 실패 패턴은 교수 출제 시 참고용이지, 채택/탈락의 직접 기준이 아님
- 새로 제출된 문제가 실패 패턴과 주제가 비슷하더라도, 문제 내용·구조가 다르면 채택할 것
- "이전에 비슷한 문제가 실패했다"는 이유만으로 탈락시키지 말 것
- 탈락은 반드시 문제 자체의 결함(강의 범위 이탈, 명백한 중복)에 근거해야 함
...
[공통 시나리오 — 그룹 문제인 경우 반드시 이것을 사용]
{shared_scenario}

[구조 규칙 — 절대 준수]
- 공통 시나리오가 있는 경우(위가 "없음"이 아닌 경우):
    scenario 필드 = 공통 시나리오 텍스트를 그대로 복사
    content 필드 = 위 시나리오를 바탕으로 이 단원 방법론에 대한 질문만 작성
- 공통 시나리오가 없는 경우(위가 "없음"):
    scenario 필드 = 직접 창작한 기업·팀 배경 사례 (3~5문장, 질문 없음)
    content 필드 = "위 사례를 참고하여..." 로 시작하는 질문만 작성
- content에 "(문제 1)", "(문제 2)", "(소1)", "(소2)" 등 소문항 구분 절대 금지
- 질문은 반드시 하나의 단일 물음이어야 함 — 복합 소문항 구조 금지
- 대문제 번호·구조는 content에 넣지 말 것 (문서 편집 단계에서 처리됨)
- scenario·content 모두 학생이 읽는 평문(plain text). JSON·딕셔너리 절대 금지.

문제 작성 형식 (JSON):
{{
  "type": "application",
  "topic": "단원명",
  "scenario": "기업/팀 배경 사례 (공통 시나리오가 있으면 그대로 복사, 없으면 직접 창작)",
  "content": "위 사례를 참고하여 [방법론]을 적용해 [구체적 물음]을 서술하시오.",
...
규칙:
- 단원 추가·삭제, short_answer/essay/application 수 변경, 난이도 조정을 처리
- 변경하지 않는 필드는 원본 그대로 유지
- group_id는 절대 추가·수정·삭제하지 말 것 (그룹 배정은 별도 단계에서 처리됨)
...
QUESTION_EDITOR = """
당신은 시험 문제 편집 전문가입니다.
아래 원본 문제를 사용자의 요구사항에 맞게 수정하십시오.
...
규칙:
- 강의 범위를 벗어나지 말 것
- 문제 유형(단답형/서술형/사례적용형)을 바꾸지 말 것
- 사례적용형이면 scenario와 content의 구분 유지 — content는 "위 사례를 참고하여..."로 시작
- content에 소문항 구분 절대 금지 ((문제 1), (소1) 등)
- 수정하지 않아도 되는 필드는 원본 그대로 유지
- 사용자가 배점 변경을 요구하지 않으면 score는 그대로 유지

반드시 JSON으로 반환:
{{
  "scenario": "수정된 시나리오 (사례적용형이 아니거나 변경 없으면 원본 그대로)",
  "content": "수정된 문제 내용",
  "intended_answer": "수정된 의도 답안",
  "score": 배점(정수),
  "edit_summary": "변경 내용 한 줄 한국어 요약"
}}
""".strip()
...
{{
  ...
  "key_concepts": ["키워드1", ...],
  "rubric": [
    {{"item": "채점항목", "points": 점수(정수), "criteria": "채점 기준 (점수 수치 언급 금지 — '~하면 인정', '~포함 시 인정' 형식으로만 작성)"}}
  ]
}}

주의: rubric criteria에 "X점", "X점 배점", "부분 점수 X점" 등 점수 수치를 절대 포함하지 마십시오.
rubric item에 "[문제N]", "소문항N" 등 편집 메타 표현 포함 금지.
""".strip()
